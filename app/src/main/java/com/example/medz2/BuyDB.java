...
@Database(entities = {BuyModal.class}, version = 1)
public abstract class BuyDB extends RoomDatabase {

    // below line is to create instance
    // for our database class.
    private static BuyDB instance;

    // below line is to create
    // abstract variable for dao.
    public abstract Buy_Dao Dao();

    // on below line we are getting instance for our database.
    public static synchronized BuyDB getInstance(Context context) {
    ...
    private static class PopulateDbAsyncTask extends AsyncTask<Void, Void, Void> {
        PopulateDbAsyncTask(BuyDB instance) {
            Buy_Dao dao = instance.Dao();
        }
        @Override
